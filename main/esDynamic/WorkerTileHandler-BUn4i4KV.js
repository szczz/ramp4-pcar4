import { mz as Le, aO as Ae, iB as je, mA as qe, jT as Ye, M as Ge } from "./main-lm2DHsZh.js";
import { E as _e, I as ae } from "./enums-qHpGJ28Q.js";
import { h as L, s as be, o as oe, r as se, f as $e, a as Ve, e as ke, i as Ze, c as Y, p as ye, d as Je, g as Qe, _ as Xe } from "./GeometryUtils-CFDcE7Ev.js";
import { t as Ce } from "./Rect-DD6XS68q.js";
import { o as Ee } from "./config-nuMERBvb.js";
import { l as ie, m as k, o as Se, n as Pe, p as et, u as we, a as ee } from "./StyleDefinition-Dfu7Kx8O.js";
import { a as tt, c as st, i as it } from "./TurboLine-BnUKJRDv.js";
import { s as rt, e as nt } from "./SourceLayerData-DDgucYFY.js";
import { t as v, U as at, O as ot, l as lt } from "./StyleRepository-BYhbefF3.js";
import { i as ve } from "./earcut-D5euH4Nc.js";
import { C as ht } from "./BidiEngine-rXjljmDh.js";
function ct(u) {
...
let Ke = class {
  ...
  getShaping(t, e, i) {
    ...
    for (const g of t) {
      ...
      o.push({ codePoint: I, x: l, y: c, vertical: w, glyphMosaicItem: _ }), _ && (l += _.metrics.advance + s);
    }
    let f = l;
    r > 0 && (f = l / Math.max(1, Math.ceil(l / r)));
    const d = t.includes("​"), h = [], y = o.length;
    for (let g = 0; g < y - 1; g++) {
      ...
      if (ft(I) || w) {
        ...
        h.push(this._buildBreak(g + 1, o[g].x, f, h, _, !1));
      }
    }
    const p = this._optimalBreaks(this._buildBreak(y, l, f, h, 0, !0));
    let M = 0;
    const x = e ? -n : n;
    let m = 0;
    for (let g = 0; g < p.length; g++) {
      ...
      if (w <= _) {
        const P = o[w].x;
        for (let T = w; T <= _; T++) o[T].x -= P, o[T].y = c;
        let D = o[_].x;
  ...
  static getTextBox(t, e) {
    ...
    for (const r of t) {
      const o = r.glyphMosaicItem.metrics.advance, l = r.x, c = r.y - He, f = l + o, d = c + e;
      i = Math.min(i, l), n = Math.max(n, f), s = Math.min(s, c), a = Math.max(a, d);
    }
  ...
  static getBox(t) {
    ...
    for (const a of t) {
      const { height: r, left: o, top: l, width: c } = a.glyphMosaicItem.metrics, f = a.x, d = a.y - (r - Math.abs(l)), h = f + c + o, y = d + r;
      e = Math.min(e, f), s = Math.max(s, h), i = Math.min(i, d), n = Math.max(n, y);
    }
  ...
  static addDecoration(t, e) {
    ...
    for (let o = 1; o < i; o++) {
      const l = t[o];
      if (l.y !== a) {
        const c = t[o - 1].x + t[o - 1].glyphMosaicItem.metrics.left + t[o - 1].glyphMosaicItem.metrics.width;
        t.push({ codePoint: 0, x: n, y: a + e - s, vertical: !1, glyphMosaicItem: { sdf: !0, rect: new Ce(4, 0, 4, 8), metrics: { width: c - n, height: 2 + 2 * s, left: 0, top: 0, advance: 0 }, page: 0, code: 0 } }), a = l.y, n = l.x + l.glyphMosaicItem.metrics.left;
      }
    }
  ...
  _buildBreak(t, e, i, s, n, a) {
    ...
    for (const l of s) {
      const c = e - l.x, f = this._breakScore(c, i, n, a) + l.score;
      f <= o && (r = l, o = f);
    }
...
let Fe = class {
  constructor(t, e, i, s, n, a, r, o, l, c, f, d) {
    this.tl = t, this.tr = e, this.bl = i, this.br = s, this.mosaicRect = n, this.labelAngle = a, this.minAngle = r, this.maxAngle = o, this.anchor = l, this.minzoom = c, this.maxzoom = f, this.page = d;
  }
}, ze = class {
  constructor(t) {
    this.shapes = t;
  ...
  _addIconPlacement(t, e, i, s, n) {
    ...
    const d = i.rect, h = 2 / a, y = c - h, p = f - h, M = y + d.width / a, x = p + d.height / a, m = new L(y, p), g = new L(M, x), I = new L(y, x), w = new L(M, p);
    if (n !== 0) {
    ...
    if (t.shapes.push(_), (!s.allowOverlap || !s.ignorePlacement) && Ee) {
      const P = s.size, D = s.padding, T = { xTile: e.x, yTile: e.y, dxPixels: c * P - D, dyPixels: f * P - D, hard: !s.optional, partIndex: 0, width: r * P + 2 * D, height: o * P + 2 * D, angle: n, minLod: X, maxLod: se };
  ...
  getTextPlacement(t, e, i, s) {
    ...
    let c = X;
    const f = r ? t.angle : 0, d = t.segment >= 0 && r, h = s.allowOverlap && s.ignorePlacement ? null : [], y = [], p = 4, M = !d;
    ...
    for (const b of e) {
      ...
      if (h && M) {
        if (D !== void 0 && D !== b.y) {
          ...
          h.push(j), x = Number.POSITIVE_INFINITY, m = Number.NEGATIVE_INFINITY, g = x, I = m;
        }
        D = b.y;
      }
      const Z = [];
      if (d) {
        ...
        if (c = this._placeGlyph(t, c, F, i, t.segment, 1, b.vertical, O, Z), o && (c = this._placeGlyph(t, c, F, i, t.segment, -1, b.vertical, O, Z)), c >= Te) break;
      ...
      for (const S of Z) {
        ...
        let re = 0, ne = 256;
        if (d && P ? b.vertical ? S.alternateVerticalGlyph ? (re = 32, ne = 96) : (re = 224, ne = 32) : (re = 224, ne = 96) : (re = 192, ne = 64), y.push(new Fe(F, $, G, j, V, S.labelAngle, re, ne, S.anchor, S.minzoom, S.maxzoom, S.page)), h && (!w || this._legible(S.labelAngle))) {
    ...
    const A = new ze(y);
    return h && h.length > 0 && (A.textColliders = h), A;
  }
  _legible(t) {
    const e = $e(t);
    return e < 65 || e >= 193;
  ...
  _placeGlyph(t, e, i, s, n, a, r, o, l) {
    let c = a;
    const f = c < 0 ? Ve(t.angle + oe, ke) : t.angle;
    let d = 0;
    i < 0 && (c *= -1, i *= -1, d = oe), c > 0 && ++n;
    let h = new L(t.x, t.y), y = s[n], p = se;
    if (s.length <= n) return p;
    for (; ; ) {
      const M = y.x - h.x, x = y.y - h.y, m = Math.sqrt(M * M + x * x), g = Math.max(i / m, e), I = M / m, w = x / m, _ = Ve(Math.atan2(w, I) + d, ke);
      if (l.push(new pe(h, f, _, o, !1, g, p)), r && l.push(new pe(h, f, _, o, !0, g, p)), g <= e) return g;
      h = y.clone();
      do {
        if (n += c, s.length <= n || n < 0) return g;
        y = s[n];
      } while (h.isEqual(y));
      let P = y.x - h.x, D = y.y - h.y;
      const T = Math.sqrt(P * P + D * D);
      P *= m / T, D *= m / T, h.x -= P, h.y -= D, p = g;
    }
  }
}, te = class extends v {
  constructor() {
  ...
  add(t, e, i, s, n, a, r, o, l, c, f, d) {
    const h = this.array;
    let y = v.i1616to32(t, e);
    h.push(y);
    const p = 31;
    y = v.i8888to32(Math.round(p * i), Math.round(p * s), Math.round(p * n), Math.round(p * a)), h.push(y), y = v.i8888to32(Math.round(p * r), Math.round(p * o), Math.round(p * l), Math.round(p * c)), h.push(y), y = v.i1616to32(f, 0), h.push(y), d && h.push(...d);
  }
}, yt = class extends v {
  constructor(t) {
    super(t);
...
class gt extends v {
  ...
  add(t, e, i, s, n, a, r) {
    ...
    let c = v.i1616to32(t, e);
    o.push(c);
    const f = 15;
    return c = v.i8888to32(Math.round(f * i), Math.round(f * s), n, a), o.push(c), r && o.push(...r), l;
  }
}
class Ne extends v {
  constructor(t) {
  ...
  add(t, e, i, s, n, a, r, o, l, c, f, d) {
    const h = this.array;
    let y = v.i1616to32(t, e);
    h.push(y), y = v.i1616to32(Math.round(8 * i), Math.round(8 * s)), h.push(y), y = v.i8888to32(n / 4, a / 4, o, l), h.push(y), y = v.i8888to32(0, $e(r), 10 * c, Math.min(10 * f, 255)), h.push(y), d && h.push(...d);
  }
}
class pt extends v {
  constructor(t) {
...
let _t = class extends me {
  ...
  processFeatures(t) {
    ...
    for (const a of this._features) {
      ...
      for (const l of r) if (l) for (const c of l) {
        const f = e.index;
        e.add(c.x, c.y, 0, 0, o), e.add(c.x, c.y, 0, 1, o), e.add(c.x, c.y, 1, 0, o), e.add(c.x, c.y, 1, 1, o), i.add(f, f + 1, f + 2), i.add(f + 1, f + 2, f + 3), this._circleIndexCount += 6;
      }
    }
  }
  serialize() {
  ...
  processFeatures(t) {
    ...
    let c = !o && e.getPaintValue("fill-antialias", i);
    if (e.outlineUsesFillColor) {
      if (c && !e.hasDataDrivenOpacity) {
        const h = e.getPaintValue("fill-opacity", i), y = e.getPaintValue("fill-opacity", i + 1);
        h < 1 && y < 1 && (c = !1);
      }
      if (c && !e.hasDataDrivenColor) {
        const h = e.getPaintValue("fill-color", i), y = e.getPaintValue("fill-color", i + 1);
        h[3] < 1 && y[3] < 1 && (c = !1);
      }
    }
    ...
    if (l) {
      ...
      for (const y of f) {
        ...
        if (!M?.rect) continue;
        const x = s.encodeAttributes(y, i, e, M), m = c && r ? n.encodeAttributes(y, i, e) : [], g = y.getGeometry(t);
        h.push({ ddFillAttributes: x, ddOutlineAttributes: m, page: M.page, geometry: g }), h.sort((I, w) => I.page - w.page);
        for (const { ddFillAttributes: I, ddOutlineAttributes: w, page: _, geometry: P } of h) this._processFeature(P, c, e.outlineUsesFillColor, I, w, d, _);
      }
    } else for (const h of f) {
      const y = a ? s.encodeAttributes(h, i, e) : null, p = c && r ? n.encodeAttributes(h, i, e) : null, M = h.getGeometry(t);
      this._processFeature(M, c, e.outlineUsesFillColor, y, p, d);
    }
  }
  serialize() {
    let t = 10;
  ...
  _processFeature(t, e, i, s, n, a, r) {
    ...
    let f;
    for (let d = 0; d < o; d++) {
      const h = We._area(t[d]);
      h > c ? (f !== void 0 && this._processFill(t, f, s, a, r), f = [d]) : h < -c && f !== void 0 && f.push(d);
    }
  ...
  _processOutline(t, e) {
    ...
    let a, r, o;
    const l = new L(0, 0), c = new L(0, 0), f = new L(0, 0);
    let d = -1, h = -1, y = -1, p = -1, M = -1, x = !1;
    const m = 0;
    let g = t.length;
    if (g < 2) return;
    const I = t[m];
    ...
    if (!(g - m < 2)) {
      for (let _ = m; _ < g; ++_) {
        _ === m ? (a = t[g - 1], r = t[m], o = t[m + 1], l.assignSub(r, a), l.normalize(), l.rightPerpendicular()) : (a = r, r = o, o = _ !== g - 1 ? t[_ + 1] : t[m], l.assign(c));
        const P = this._isClipEdge(a, r);
        p === -1 && (x = P), c.assignSub(o, r), c.normalize(), c.rightPerpendicular();
        const D = l.x * c.y - l.y * c.x;
        f.assignAdd(l, c), f.normalize();
        const T = -f.x * -l.x + -f.y * -l.y;
        let B = Math.abs(T !== 0 ? 1 / T : 1);
        B > 8 && (B = 8), D >= 0 ? (y = i.add(r.x, r.y, l.x, l.y, 0, 1, e), p === -1 && (p = y), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), h = i.add(r.x, r.y, B * -f.x, B * -f.y, 0, -1, e), M === -1 && (M = h), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), d = h, h = y, y = i.add(r.x, r.y, f.x, f.y, 0, 1, e), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), h = i.add(r.x, r.y, c.x, c.y, 0, 1, e), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y)) : (y = i.add(r.x, r.y, B * f.x, B * f.y, 0, 1, e), p === -1 && (p = y), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), h = i.add(r.x, r.y, -l.x, -l.y, 0, -1, e), M === -1 && (M = h), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), d = h, h = y, y = i.add(r.x, r.y, -f.x, -f.y, 0, -1, e), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y), d = i.add(r.x, r.y, -c.x, -c.y, 0, -1, e), d >= 0 && h >= 0 && y >= 0 && !P && s.add(d, h, y));
      }
      d >= 0 && h >= 0 && p >= 0 && !x && s.add(d, h, p), d >= 0 && p >= 0 && M >= 0 && !x && s.add(d, M, p), this._outlineIndexCount += 3 * (s.index - n);
    }
  }
  _processFill(t, e, i, s, n) {
    ...
    for (const f of e) {
      const d = t[f], h = d.length;
      for (let y = 0; y < h; ++y) l.push(d[y].x, d[y].y);
    }
    const c = ve(l, a, 2);
    if (s && ve.deviation(l, a, 2, c) > 0) {
      const f = e.map((y) => t[y].length), { buffer: d, vertexCount: h } = tt(l, f);
      if (h > 0) {
        const y = this._fillVertexBuffer.index;
        for (let p = 0; p < h; p++) this._fillVertexBuffer.add(d[2 * p], d[2 * p + 1], i);
        for (let p = 0; p < h; p += 3) {
          const M = y + p;
        ...
        if (n !== void 0) {
          ...
          M ? M[1] += h : p.set(n, [this._fillIndexStart + this._fillIndexCount, h]);
        }
        this._fillIndexCount += h;
      }
    } else {
      const f = c.length;
      if (f > 0) {
        ...
        let h = 0;
        for (; h < o; ) this._fillVertexBuffer.add(l[h++], l[h++], i);
        let y = 0;
        for (; y < f; ) this._fillIndexBuffer.add(d + c[y++], d + c[y++], d + c[y++]);
        if (n !== void 0) {
          const p = this._patternMap, M = p.get(n);
...
class Mt extends me {
  ...
  getResources(t, e, i) {
    ...
    if (!a && !r) return;
    const l = o?.getValue(n) || 0, c = o?.isDataDriven, f = a?.isDataDriven, d = r?.isDataDriven;
    if (f || d) for (const h of this._features) e(f ? a.getValue(n, h) : this._getDashArrayKey(h, n, s, r, c, o, l));
    else if (a) e(a.getValue(n));
    else if (r) {
      const h = r.getValue(n);
      e(s.getDashKey(h, l));
    }
  }
  processFeatures(t) {
    ...
    t && t.setExtent(this.layerExtent);
    const o = e.getPaintProperty("line-pattern"), l = e.getPaintProperty("line-dasharray"), c = o?.isDataDriven, f = l?.isDataDriven;
    let d;
    d = e.getLayoutProperty("line-cap");
    const h = d?.isDataDriven ? d : null, y = h ? null : e.getLayoutValue("line-cap", i), p = y || 0, M = !!h;
    ...
    if (c || f) {
      const A = [];
      for (const b of s) {
        const E = c ? o.getValue(i, b) : this._getDashArrayKey(b, i, e, l, M, h, p), V = this._spriteInfo[E];
        if (!V?.rect) continue;
        const C = r.encodeAttributes(b, i, e, V), O = b.getGeometry(t);
        A.push({ ddAttributes: C, page: V.page, cap: h ? h.getValue(i, b) : y, join: x ? x.getValue(i, b) : m, miterLimit: g ? g.getValue(i, b) : I, roundLimit: w ? w.getValue(i, b) : _, halfWidth: 0.5 * (P ? P.getValue(i, b) : D), offset: T ? T.getValue(i, b) : B, geometry: O });
      }
      ...
      for (const A of s) {
        ...
        h && (n.capType = h.getValue(i, A)), x && (n.joinType = x.getValue(i, A)), g && (n.miterLimit = g.getValue(i, A)), w && (n.roundLimit = w.getValue(i, A)), P && (n.halfWidth = 0.5 * P.getValue(i, A)), T && (n.offset = T.getValue(i, A));
  ...
  _processGeometry(t, e, i) {
    ...
    if (t.length < 2) return;
    const l = this.tessellationProperties._lineIndexBuffer, c = 3 * l.index;
    ...
    const f = 3 * l.index - c;
    if (i !== void 0) {
      const d = this._patternMap, h = d.get(i);
      h ? h[1] += f : d.set(i, [c + this._lineIndexCount, f]);
    }
...
const Pt = (u) => (t, e, i, s, n, a, r, o, l, c, f) => (u._lineVertexBuffer.add(t, e, r, o, i, s, n, a, l, c, f, u._ddValues), u._lineVertexBuffer.index - 1), wt = (u) => (t, e, i) => {
...
class z extends me {
  ...
  getResources(t, e, i) {
    ...
    const c = [];
    let f, d, h, y;
    a && !a.isDataDriven && (f = a.getValue(n)), r && !r.isDataDriven && (d = r.getValue(n)), o && o.isDataDriven || (h = s.getLayoutValue("text-transform", n), o = null), l && l.isDataDriven || (y = s.getLayoutValue("text-font", n), l = null);
...
`), o && (h = o.getValue(n, p)), h) {
  ...
  processFeatures(t) {
    t && t.setExtent(this.layerExtent);
    const e = this.layer, i = this.zoom, s = e.getLayoutValue("symbol-placement", i), n = s !== Pe.POINT, a = e.getLayoutValue("symbol-spacing", i) * le, r = e.getLayoutProperty("icon-image"), o = e.getLayoutProperty("text-field"), l = r ? new at(e, i, n) : null, c = o ? new ot(e, i, n) : null, f = this._workerTileHandler;
    let d;
    ...
    let y = 1;
    c?.size && (y = c.size / H);
    const p = c ? c.maxAngle * be : 0, M = c ? c.size * le : 0;
    for (const x of this._symbolFeatures) {
      ...
      const w = x.label;
      if (w) {
        je(c), c.update(i, x.feature);
        const _ = n && c.rotationAlignment === ie.MAP ? c.keepUpright : c.writingMode && c.writingMode.includes(et.VERTICAL);
        let P = 0.5;
        switch (c.anchor) {
          case k.TOP_LEFT:
          case k.LEFT:
        ...
        const B = c.letterSpacing * H, A = n ? 0 : c.maxWidth * H, b = c.lineHeight * H, E = x.textFont.map((V) => f.getGlyphItems(V));
      ...
      for (let _ of x.geometry) {
        ...
        if (s === Pe.LINE) {
          if (g?.length && c?.size) {
            const D = c.size * le * (2 + Math.min(2, 4 * Math.abs(c.offset[1])));
        ...
        for (const D of P) {
          if (D.x < 0 || D.x > Re || D.y < 0 || D.y > Re || n && I > 0 && c?.rotationAlignment === ie.MAP && !z._honorsTextMaxAngle(_, D, I, p, M)) continue;
          ...
          h.push(T), this._processFeature(T, l, c);
        }
      }
    }
    h.sort(Tt), this._addPlacedGlyphs(), this._symbolInstances = h;
  }
  serialize() {
    let t = 14;
    ...
    for (const [l, [c, f]] of this.markerPageMap) e[o++] = l, e[o++] = c, e[o++] = f;
    e[o++] = this.glyphsPageMap.size;
    for (const [l, [c, f]] of this.glyphsPageMap) e[o++] = l, e[o++] = c, e[o++] = f;
  ...
  _processFeature(t, e, i) {
    const { line: s, iconMosaicItem: n, shaping: a, anchor: r } = t, o = this.zoom, l = this.layer, c = !!n;
    let f = !0;
    c && (f = e?.optional || !n);
    const d = a && a.length > 0, h = !d || i?.optional;
    let y, p;
    if (c && (y = this._placementEngine.getIconPlacement(r, n, e)), (y || f) && (d && (p = this._placementEngine.getTextPlacement(r, a, s, i)), p || h)) {
      if (y && p || (h || f ? h || p ? f || y || (p = null) : y = null : (y = null, p = null)), p) {
  ...
  _addPlacedIcons(t, e, i, s, n, a) {
    const r = Math.max(i - 1, 0), o = this._iconVertexBuffer, l = this._iconIndexBuffer, c = this._markerMap;
    for (const f of e) {
      const d = n ? 0 : Math.max(i + Y(f.minzoom), r), h = n ? 25 : Math.min(i + Y(f.maxzoom), 25);
      if (h <= d) continue;
      ...
      o.add(_.x, _.y, y.x, y.y, D, T, g, I, w, d, h, a), o.add(_.x, _.y, p.x, p.y, B, T, g, I, w, d, h, a), o.add(_.x, _.y, M.x, M.y, D, A, g, I, w, d, h, a), o.add(_.x, _.y, x.x, x.y, B, A, g, I, w, d, h, a), t.iconVertexRanges.length > 0 && t.iconVertexRanges[0][0] + t.iconVertexRanges[0][1] === b ? t.iconVertexRanges[0][1] += 4 : t.iconVertexRanges.push([b, 4]), l.add(P, P + 1, P + 2), l.add(P + 1, P + 2, P + 3), c.has(s) ? c.get(s)[1] += 6 : c.set(s, [this._iconIndexStart + this._iconIndexCount, 6]), this._iconIndexCount += 6;
    }
  }
  _addPlacedGlyphs() {
  ...
  static _pushAnchors(t, e, i, s) {
    ...
    if (r *= 0.5, n <= r) return;
    const o = r / n;
    let l = 0, c = -(i = n / Math.max(Math.round(n / i), 1)) / 2;
    const f = e.length - 1;
    for (let d = 0; d < f; d++) {
      const h = e[d], y = e[d + 1], p = y.x - h.x, M = y.y - h.y, x = Math.sqrt(p * p + M * M);
      let m;
      for (; c + i < l + x; ) {
        c += i;
        const g = (c - l) / x, I = ye(h.x, y.x, g), w = ye(h.y, y.y, g);
  ...
  static _pushCenterAnchor(t, e) {
    ...
    const r = e.length - 1;
    for (let o = 0; o < r; o++) {
      const l = e[o], c = e[o + 1], f = c.x - l.x, d = c.y - l.y, h = Math.sqrt(f * f + d * d);
      if (n < a + h) {
        const y = (n - a) / h, p = ye(l.x, c.x, y), M = ye(l.y, c.y, y), x = Math.atan2(d, f);
  ...
  static _honorsTextMaxAngle(t, e, i, s, n) {
    ...
    for (; a < r; ) {
      ...
      const m = this._deviation(h, y, M);
      for (c.push({ deviation: m, distToAnchor: a }), f += m; a - c[0].distToAnchor > n; ) f -= c.shift().deviation;
  ...
  static _smoothVertices(t, e) {
    ...
    return d.push(new L(l, c)), t[0].x = r, t[0].y = o, t[i - 1].x = l, t[i - 1].y = c, d;
  }
  static _pushCentroid(t, e) {
    const r = e.length - 1;
    let o = 0, l = 0, c = 0, f = e[0].x, d = e[0].y;
    f > 4096 && (f = 4096), f < 0 && (f = 0), d > 4096 && (d = 4096), d < 0 && (d = 0);
    for (let h = 1; h < r; h++) {
      let y = e[h].x, p = e[h].y, M = e[h + 1].x, x = e[h + 1].y;
      y > 4096 && (y = 4096), y < 0 && (y = 0), p > 4096 && (p = 4096), p < 0 && (p = 0), M > 4096 && (M = 4096), M < 0 && (M = 0), x > 4096 && (x = 4096), x < 0 && (x = 0);
      const m = (y - f) * (x - d) - (M - f) * (p - d);
      o += m * (f + y + M), l += m * (d + p + x), c += m;
    }
    o /= 3 * c, l /= 3 * c, isNaN(o) || isNaN(l) || t.push(new ge(o, l));
  }
}
z._bidiEngine = new ht();
var K;
(function(u) {
...
class bt {
  ...
  async parse(t) {
    const e = it(), i = this._initialize(t), { returnedBuckets: s } = i;
    this._processLayers(i), this._linkReferences(i), this._filterFeatures(i);
    const n = [], a = /* @__PURE__ */ new Set(), r = (c, f) => {
      a.has(c) || (n.push({ name: c, repeat: f }), a.add(c));
    }, o = {};
    for (const c of s) c.getResources(c.tileClipper, r, o);
  ...
  _processLayers(t) {
    const { sourceNameToTileData: e, zoom: i, layers: s, sourceNameToTileClipper: n, sourceNameToUniqueSourceLayerBuckets: a, sourceNameToUniqueSourceLayers: r, returnedBuckets: o, layerIdToBucket: l, referencerUIDToReferencedId: c } = t, f = this._sourceDataMaxLOD;
    for (let d = s.length - 1; d >= 0; d--) {
      const h = s[d];
      if (i < f) {
        if (h.minzoom && i < Math.floor(h.minzoom) || h.maxzoom && i >= h.maxzoom) continue;
      } else if (h.maxzoom && i >= h.maxzoom) continue;
      if (h.type === ee.BACKGROUND || !this._canParseStyleLayer(h.uid) || !e[h.source] || !n[h.source]) continue;
      const y = e[h.source], p = n[h.source], M = h.sourceLayer, x = y[M];
      if (x) {
        let m = r[h.source];
        if (m || (m = r[h.source] = /* @__PURE__ */ new Set()), m.add(h.sourceLayer), h.refLayerId) c.set(h.uid, h.refLayerId);
        else {
          const g = this._createBucket(h);
          if (g) {
            g.layerUIDs = [h.uid], g.layerExtent = x.extent, g.tileClipper = p;
            let I = a[h.source];
            I || (I = a[h.source] = {});
            let w = I[M];
            w || (w = I[M] = []), w.push(g), o.push(g), l[h.id] = g;
          }
        }
      }
    }
  ...
  _filterFeatures(t) {
    ...
    for (let c = 0; c < o.length; c++) {
      const f = l[c], d = o[c];
      if (!i[f] || !s[f]) continue;
      const h = i[f][d], y = s[f][d];
      if (!y || y.length === 0) continue;
      if (Ye(e)) return;
      let p = 0;
      const M = h.getData();
      for (; M.nextTag(2); ) {
        const x = M.getMessage(), m = new rt(x, h, p++);
        x.release();
        const g = m.values;
        if (g) {
          const I = g._minzoom;
  ...
  _parseTileData(t) {
    ...
    for (const i of Object.keys(t)) {
      ...
      for (; s.next(); ) switch (s.tag()) {
        case 3: {
          const a = s.getMessage(), r = new nt(a);
...
let Dt = class {
  ...
  async parse(t, e) {
    ...
    if (i != null) {
      const c = () => {
        i.removeEventListener("abort", c), this.status = K.INVALID;
      };
    ...
    for (const c of s) if (c.hasFeatures()) {
      const f = c.serialize();
      a.push(f);
    } else r.push(c.layer.uid);
    const o = [...a];
    let l = null;
...
class qt {
  ...
  setStyle(t) {
    ...
    this._styleRepository = new lt(e), this._sourceDataMaxLOD = i, this._spriteInfo = {}, this._glyphInfo = {};
  }
  fetchSprites(t, e, i) {
...
//# sourceMappingURL=WorkerTileHandler-BUn4i4KV.js.map
