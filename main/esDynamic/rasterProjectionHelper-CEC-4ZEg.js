import { bn as V, bo as Sn, b8 as A, bp as $, bq as O, bm as Pn, b5 as mn, a3 as G, aQ as Gn, br as kn, s as Nn, bs as v, bt as T, bu as en, bv as En, bw as Tn, bx as vn, by as on } from "./main-lm2DHsZh.js";
var H;
function hn(n, t, i) {
  return !kn(n, t, i);
}
function L(n, t, i) {
  const s = hn(n, t, i);
  if (s && !V()) throw new Nn("rasterprojectionhelper-project", "projection engine is not loaded");
  return s;
}
(function(n) {
...
const sn = (n, t, i, s = 0) => {
  ...
  const { cols: f, rows: r } = t, l = (e - a) / f / i[0], w = (c - o) / r / i[1], p = 2 * s;
  let x = 0, m = !1, g = [0, 0];
  for (let u = 0; u < f - 3; u++) {
    for (let M = 0; M < r - 3; M++) {
      const y = u * r * 2 + 2 * M, h = (n[y] + n[y + 4] + n[y + 4 * r] + n[y + 4 * r + 4]) / 4, d = (n[y + 1] + n[y + 5] + n[y + 4 * r + 1] + n[y + 4 * r + 5]) / 4, R = Math.abs((h - n[y + 2 * r + 2]) / l), b = Math.abs((d - n[y + 2 * r + 3]) / w);
...
function pn(n, t, i, s) {
  ...
  if (o && a != null && e != null) for (let f = 0; f < n.length; f++) {
    ...
    if (!r) continue;
    const { x: l } = n[f], { x: w } = r;
    w >= e[1] - c && Math.abs(l - a[0]) < o ? r.x -= e[1] - e[0] : w <= e[0] + c && Math.abs(l - a[1]) < o && (r.x += e[1] - e[0]);
  }
}
function zn(n) {
...
function ln(n) {
  ...
  if (s != null && s.steps.length) {
    ...
    if (i.isGeographic && (f = e.map(([l]) => l > 179.9955 ? 1 : l < -179.9955 ? -1 : 0)), s.steps.forEach((l) => {
      const w = l.wkid ? v.geogtran(l.wkid) : v.fromString(T.PE_TYPE_GEOGTRAN, l.wkt);
      En.geogToGeog(w, e.length, e, null, l.isInverse ? T.PE_TRANSFORM_2_TO_1 : T.PE_TRANSFORM_1_TO_2);
    }), f) for (let l = 0; l < e.length; l++) {
      const w = f[l], p = e[l][0], x = p > 179.9955 ? 1 : p < -179.9955 ? -1 : 0;
      w && x && w !== x && (e[l][0] = w > 0 ? p + 360 : p - 360);
    }
  }
  if (!i.isGeographic) {
...
function $n(n) {
  ...
  let { xmin: f, ymax: r } = n;
  e && (f += s / 2, r -= a / 2);
  const l = [], w = [], p = Math.max(t, i);
  for (let m = 0; m < p; m++) {
    const g = f + s * Math.min(t, m), u = r - a * Math.min(i, m), M = $(new G({ x: g, y: u, spatialReference: o }), c);
    m <= t && l.push(M.x), m <= i && w.push(M.y);
  }
  const x = [];
  for (let m = 0; m < t; m++) for (let g = 0; g < i; g++) x.push([l[m], w[g]]);
  return x;
}
function W(n, t = !1) {
  let i = n.wkid || n.wkt2 || n.wkt;
  if (!i || n.isGeographic) return null;
  if (i = String(i), D.has(i)) {
    const o = D.get(i);
  ...
  return D.set(i, { pcs: a, gcs: e }), t ? e : a;
}
function cn(n, t = 0, i = !1) {
  ...
  for (let u = 0, M = r.length; u < M; u++) {
    ...
    if (R === h) w.push([y, d, h, R, 2]);
    else {
      const b = (d - y) / (R - h || 1e-4), N = y - b * h;
      h < R ? w.push([b, N, h, R, 0]) : w.push([b, N, R, h, 1]);
    }
    p = p < y ? p : y, x = x < h ? x : h, m = m > y ? m : y, g = g > h ? g : h;
  }
  return { isEnvelope: !1, isPannable: f, vertices: r, coef: w, bbox: [p, x, m, g], poleLocation: c };
}
function Z(n, t) {
  ...
  if (t.isEnvelope) {
    ...
    for (let M = 0, y = 0; M < s; M++) {
      const h = f + e * M, d = t.isPannable || h >= x && h <= g;
      for (let R = 0; R < a; R++, y++) {
        const b = r - o * R;
  ...
  for (let x = 0; x < a; x++) {
    ...
    if (g.length > 2) {
      ...
      M = [];
      for (let d = 1; d < u.length; d++) u[d] === 2 && d !== u.length - 1 || (u[d] !== y && (M.push(y === 0 ? Math.min(h, g[d - 1]) : Math.max(h, g[d - 1])), y = u[d], h = g[d]), d === u.length - 1 && M.push(u[d] === 0 ? Math.min(h, g[d]) : Math.max(h, g[d])));
      M.sort((d, R) => d - R);
    } else g[0] > g[1] && (M = [g[1], g[0]]);
    p.push(M);
  }
  for (let x = 0, m = 0; x < s; x++) {
    ...
    for (let u = 0; u < a; u++, m++) {
      const M = r - o * u, y = p[u];
      if (y.length === 2) g >= y[0] && g <= y[1] && (i.push([g, M]), l[m] = 1);
      else if (y.length > 2) {
        let h = !1;
...
function yn(n, t, i = null, s = !0) {
  ...
  if (e === 0 || o == null || c == null) {
    const r = fn(n, t, i, s);
    if (o == null && c != null && Math.abs(r.width - c) < k(t) && V()) {
      const l = W(a);
      if (l != null && l.poleLocation === H.None && n.width < (l.bbox[2] - l.bbox[0]) / 2) return On(n, t) || r;
    }
    return r;
  }
  ...
  if (f.length === 1 && n.xmax < o && n.xmax - o / 2 > k(a)) {
    ...
    for (let w = 0; w <= e; w++) {
      const p = w === 0 ? r : -o / 2, x = w === e ? l - o * w : o / 2;
      f[w] = new A({ xmin: p, xmax: x, ymin: n.ymin, ymax: n.ymax, spatialReference: a });
    }
  }
...
function On(n, t) {
  ...
  const { rings: l } = r, w = k(c), p = new A({ spatialReference: t });
  for (let x = 0; x < 2; x++) {
    ...
    else if (p.ymin = Math.min(p.ymin, a), p.ymax = Math.max(p.ymax, o), Math.abs(e - i / 2) < w) p.xmin = s, p.xmax = p.xmax + i;
    else {
      if (!(Math.abs(s + i / 2) < w)) return null;
      p.xmax = e + i;
    }
  }
  return p;
...
function fn(n, t, i = null, s = !0, a = !0) {
  ...
  const r = k(e, 1e-3), l = k(e, Y), w = k(t, 1e-3);
  if (Math.abs(o.xmin - f[0]) < w && Math.abs(o.xmax - f[1]) < w) {
...
function qn(n) {
  ...
  let l = a.x;
  const w = [], p = n.transform != null && n.transform.type === "gcs-shift", x = t + (p ? 0 : s), m = p ? i - t : s - t;
  for (let g = 0; g <= c; g++) {
    const u = (o.xmax - t) / l / f, M = u - Math.floor(u) == 0 ? u : Math.ceil(u), y = m / l / f, h = y - Math.floor(y) == 0 ? y : Math.ceil(y), d = Math.floor(x / l / f), R = Math.round(x / l) % f, b = (f - Math.round(m / l) % f) % f;
    w.push({ resolutionX: l, blockWidth: f, datasetColumnCount: M, worldColumnCountFromOrigin: h, leftMargin: R, rightPadding: b, originColumnOffset: d }), l *= r;
  }
  return { originX: t, halfWorldWidth: s, pyramidsInfo: w, hasGCSSShiftTransform: p };
}
function In(n) {
  if (!n || n.isGeographic) return n;
  const t = String(n.wkid || n.wkt2 || n.wkt);
  let i;
  return Q.has(t) ? i = Q.get(t) : (i = (n.wkid ? v.coordsys(n.wkid) : v.fromString(T.PE_TYPE_PROJCS, n.wkt2 || n.wkt)).getGeogcs().getCode(), Q.set(t, i)), new vn({ wkid: i });
}
function Jn(n) {
  const t = n.isAdaptive && n.spacing == null;
  let i = n.spacing || [B, B], s = U(n), a = { cols: s.size[0] + 1, rows: s.size[1] + 1 };
...
function U(n) {
  const { projectedExtent: t, srcBufferExtent: i, pixelSize: s, datumTransformation: a, rasterTransform: e } = n, o = t.spatialReference, c = i.spatialReference, f = L(o, c), { xmin: r, ymin: l, xmax: w, ymax: p } = t, x = C(c), m = x != null && (n.hasWrapAround || e?.type === "gcs-shift"), g = n.spacing || [B, B], u = g[0] * s.x, M = g[1] * s.y, y = g[0] === 1, h = Math.ceil((w - r) / u - 0.1 / g[0]) + (y ? 0 : 1), d = Math.ceil((p - l) / M - 0.1 / g[1]) + (y ? 0 : 1), R = zn({ cols: h, rows: d, xmin: r, ymax: p, xres: u, yres: M, inSR: o, outSR: c, datumTransformation: a, preferPE: g[0] <= F, usePixelCenter: y }), b = [];
  let N, P = 0;
  const E = y ? -1 : NaN, { xmin: z, xmax: I, ymax: J, width: wn, height: Mn } = i, Rn = k(c, Y), bn = x != null && z > 0 && I > x / 2;
  let nn = !1;
  if (f) {
    const _ = W(o);
  ...
  for (let _ = 0; _ < h; _++) {
    ...
    for (let j = 0; j < d; j++) {
      let S = R[_ * d + j];
      if (m && S[0] > I && S[0] > x / 2 - Rn ? S[0] -= x : m && _ === 0 && S[0] < 0 && bn && !e && (S[0] += x), !S || isNaN(S[0]) || isNaN(S[1])) b.push(E), b.push(E), K.push(null), P++;
      else {
        if (e) {
          const tn = e.inverseTransform(new G({ x: S[0], y: S[1], spatialReference: c }));
          S = [tn.x, tn.y];
        }
        K.push(S), _ > 0 && m && N[j] && S[0] < N[j][0] && (S[0] += x, nn && S[0] > I && S[0] > x && (S[0] -= x)), b.push((S[0] - z) / wn), b.push((J - S[1]) / Mn);
      }
    }
    N = K;
  }
...
function xn(n, t, i) {
  ...
  for (let f = 0; f < s - 1; f++) {
    for (let r = 0; r < a - 1; r++) {
      let l = f * a * 2 + 2 * r;
      const w = n[l], p = n[l + 1], x = n[l + 2], m = n[l + 3];
      l += 2 * a;
      const g = n[l], u = n[l + 1], M = n[l + 2], y = n[l + 3];
      let h = 0, d = 12 * (r * (s - 1) + f);
      for (let R = 0; R < 3; R++) e[d++] = o[h++] * w + o[h++] * x + o[h++] * M;
      h = 0;
      for (let R = 0; R < 3; R++) e[d++] = o[h++] * p + o[h++] * m + o[h++] * y;
      h = 0;
      for (let R = 0; R < 3; R++) e[d++] = c[h++] * w + c[h++] * g + c[h++] * M;
      h = 0;
...
function Xn(n, t, i) {
  ...
  if (c != null && c.length) {
    ...
    const M = e === 0 ? a : c[e - 1];
    return o && Math.min(M.x, M.y) * O(t.spatialReference) > 19567 && (o = !1), { pyramidLevel: e, pyramidResolution: new G({ x: M.x, y: M.y, spatialReference: t.spatialReference }), excessiveReading: o };
  }
  const l = Math.log(n.x / a.x) / Math.LN2, w = Math.log(n.y / a.y) / Math.LN2, p = t.storageInfo.maximumPyramidLevel || 0;
  e = i === "down" ? Math.floor(Math.min(l, w)) : i === "up" ? Math.ceil(Math.max(l, w)) : Math.round((l + w) / 2), e < 0 ? e = 0 : e > p && (o = e > p + 3, e = p);
  const x = 2 ** e;
...
function Dn(n, t, i) {
  ...
  const r = (f.x + f.y) / 2, l = O(t), w = r * l * 96 * 39.37, p = t.isGeographic ? 256 / s * 2958287637958547e-7 : 256 / s * 591657527591555e-6;
  let x = n.dataType === "vector-magdir" || n.dataType === "vector-uv";
  const m = An(n, t), g = Math.min(Math.ceil(Math.log(Math.min(n.width, n.height) / 32) / Math.LN2), Math.ceil(Math.log(p / 2 / w) / Math.LN2));
  ...
  if (x) {
    ...
    u = an(new G({ x: P, y: P, spatialReference: { wkid: z } }), o, m), u.x *= M / E, u.y *= M / E;
  } else {
    u = { x: c.x, y: c.y };
    let P = 0;
    for (; M < p * (y / 2) && P < g; ) P++, M *= h, u.x *= h, u.y *= h;
    Math.max(M, p) / Math.min(M, p) <= y && (M = p);
  }
  const d = [M], R = [{ x: u.x, y: u.y }], b = 70.5310735, N = Math.min(b, w) / y;
  for (; M >= N; ) M /= h, u.x /= h, u.y /= h, d.push(M), R.push({ x: u.x, y: u.y });
...
//# sourceMappingURL=rasterProjectionHelper-CEC-4ZEg.js.map
