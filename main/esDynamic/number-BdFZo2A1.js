import { aP as E, aQ as S } from "./main-BVGiskS7.js";
...
function w(e = E()) {
  ...
  const [n, r, s = "#,##0.###"] = x[e];
  return { decimal: n, group: r, pattern: s };
}
function I(e, t) {
  const n = w((t = { ...t }).locale);
  t.customs = n;
  const r = t.pattern || n.pattern;
  return isNaN(e) || Math.abs(e) === 1 / 0 ? null : v(e, r, t);
}
const N = /[#0,]*[#0](?:\.0*#*)?/;
function v(e, t, n) {
  const r = (n = n || {}).customs.group, s = n.customs.decimal, l = t.split(";"), o = l[0];
  ...
  return n.fractional === !1 && (n.places = 0), t.replace(d, z(e, u[0], { decimal: s, group: r, places: n.places, round: n.round }));
}
function z(e, t, n) {
  (n = n || {}).places === !0 && (n.places = 0), n.places === 1 / 0 && (n.places = 6);
  const r = t.split("."), s = typeof n.places == "string" && n.places.indexOf(",");
  let l = n.places;
  ...
  if (r[1] || n.places) {
    ...
    const c = n.places !== void 0 ? n.places : r[1] && r[1].lastIndexOf("0") + 1;
  ...
  let p, i, a = r[0].lastIndexOf(",");
  if (a !== -1) {
    p = r[0].length - a - 1;
    const c = r[0].substr(0, a);
  ...
  return o[0] = f.reverse().join(n.group || ","), o.join(n.decimal || ".");
}
function O(e) {
  const t = w((e = e || {}).locale), n = e.pattern || t.pattern, r = t.group, s = t.decimal;
  let l = 1;
  if (n.includes("%")) l /= 100;
  else if (n.includes("‰")) l /= 1e3;
  ...
  return o.length === 1 && o.push("-" + o[0]), { regexp: h(o, (u) => (u = "(?:" + S(u, ".") + ")").replace(N, (g) => {
    const p = { signed: !1, separator: e.strict ? r : [r, ""], fractional: e.fractional, decimal: s, exponent: !1 }, i = g.split(".");
    let a = e.places;
    ...
    const f = i[0].split(",");
    return f.length > 1 && (p.groupSize = f.pop().length, f.length > 1 && (p.groupSize2 = f.pop().length)), "(" + y(p) + ")";
  }), !0).replaceAll(/[\xa0 ]/g, "[\\s\\xa0]"), group: r, decimal: s, factor: l };
}
function A(e, t) {
  const n = O(t), r = new RegExp("^" + n.regexp + "$").exec(e);
  if (!r) return NaN;
  let s = r[1];
  if (!r[1]) {
    if (!r[2]) return NaN;
...
function y(e) {
  ...
  const t = b(e), n = h(e.fractional, (s) => {
    let l = "";
    return s && e.places !== 0 && (l = "\\" + e.decimal, e.places === 1 / 0 ? l = "(?:" + l + "\\d+)?" : l += "\\d{" + e.places + "}"), l;
  }, !0);
  let r = t + n;
  return n && (r = "(?:(?:" + r + ")|(?:" + n + "))"), r + h(e.exponent, (s) => s ? "([eE]" + b({ signed: e.eSigned }) + ")" : "");
}
function b(e) {
  return "signed" in (e = e || {}) || (e.signed = [!0, !1]), "separator" in e ? "groupSize" in e || (e.groupSize = 3) : e.separator = "", h(e.signed, (t) => t ? "[-+]" : "", !0) + h(e.separator, (t) => {
    ...
    if (r) {
      const s = "(?:0|[1-9]\\d{0," + (r - 1) + "}(?:[" + t + "]\\d{" + r + "})*[" + t + "]\\d{" + n + "})";
      return n - r > 0 ? "(?:" + s + "|(?:0|[1-9]\\d{0," + (n - 1) + "}))" : s;
    }
...
//# sourceMappingURL=number-BdFZo2A1.js.map
