...
import { s as P, ao as C, dq as G, dr as E, ds as x, dt as D } from "./main-lm2DHsZh.js";
...
function* v(t, n = {}) {
  ...
  for (const r of t) {
    const { geometry: s, properties: p, id: i } = r;
    if (s && b(s.type) !== o) continue;
    const l = p || {};
    let c;
    e && (c = l[e], i == null || c || (l[e] = c = i)), yield new x(s ? H(new D(), s, n) : null, l, null, c ?? void 0);
  }
}
function A(t) {
...
function tt(t, n = {}) {
  ...
  let s, p = !1, i = null, l = !1, { geometryType: c = null } = n, m = !1;
  for (const d of M(t)) {
    const { geometry: h, properties: u, id: a } = d;
    if ((!h || (c || (c = b(h.type)), b(h.type) === c)) && (p || (p = A(R(h))), l || (l = a != null, l && (s = typeof a, u && (i = Object.keys(u).filter((y) => u[y] === a)))), u && i && l && a != null && (i.length > 1 ? i = i.filter((y) => u[y] === a) : i.length === 1 && (i = u[i[0]] === a ? i : [])), !m && u)) {
      let y = !0;
      for (const f in u) {
        if (e.has(f)) continue;
        const F = u[f];
  ...
  if (w) {
    for (const d of o) if (d.name === w && E(d)) {
      d.type = "esriFieldTypeOID";
      break;
    }
  }
  return { fields: o, geometryType: c, hasZ: p, objectIdFieldName: w, objectIdFieldType: s, unknownFields: Array.from(r) };
}
function nt(t, n) {
...
//# sourceMappingURL=geojson-CvVDWXCl.js.map
