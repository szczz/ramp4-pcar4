import { bY as I } from "./main-lm2DHsZh.js";
import { a as l } from "./fabric-Yk5c5EFe.js";
const x = 30, M = 20, N = 16, T = 12, G = 8, u = 32, d = 32, L = 350, w = 20, m = "Montserrat, BlinkMacSystemFont, Segoe UI, Helvetica, Arial, sans-serif";
class H extends I {
  get config() {
  ...
  async make(o) {
    ...
    const c = (await Promise.all(this._makeSegments(i, e))).map(({ title: s, items: f }, b) => {
      b > 0 && (n += x), s.top = n, n += s.height + M;
      const p = f.map(({ title: h, items: y }, C) => {
        const S = [];
        return h && !(f.length === 1 && h.text === s.text) && (C > 0 && (n += N), h.top = n, n += h.height + T, S.push(h)), y.forEach((g) => {
...
//# sourceMappingURL=index-DmNcj-xt.js.map
