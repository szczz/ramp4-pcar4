import { a9 as _, cr as X, J as Y, jg as N, b0 as A, jh as E, b1 as d, $ as f, dK as u, fj as Z, ah as ss, cq as es, ak as S, fl as ts, al as os, _ as as, f8 as ns, D as rs } from "./main-lm2DHsZh.js";
import { h as is, a as cs, i as k, o as x, A as ls } from "./MeshTransform-Vd-J1X5A.js";
import { r as us, n as ps, a as ms, d as fs, c as D, l as ds, t as hs, u as gs, p as ws } from "./uploadAssetErrors-BsmhXTgE.js";
...
let ys = class {
  ...
  emitProgress() {
    ...
    for (const [s, o] of this._progressMap.entries())
      e += o * this._weights[s];
    if (e === 1 && _("enable-feature:esri-3dofl-upload-timings")) {
  ...
  setProgress(e, s) {
    if (this._progressMap.set(e, s), _("enable-feature:esri-3dofl-upload-timings")) {
...
const bs = 10, As = 10, B = 8e-6, $s = A(50), js = A(1e3), O = 1e6, I = 20 * O, Ms = 2e9, Fs = 3;
async function vs({ data: t, name: e, description: s }, o, a) {
  ...
  try {
    ...
    u(a);
    const l = Z(o), m = c.maxUploadFileSize * O, g = l ? Ms : m, b = l ? Math.min(I, m) : I;
    if (t.size > g) throw new Error("Data too large");
    const L = d(r, "register"), { data: j } = await f(L, { query: { f: "json", itemName: _s(e), description: s }, responseType: "json", method: "post" });
    if (u(a), !j.success) throw new Error("Registration failed");
    const { itemID: H } = j.item;
    n = d(r, H);
    const G = d(n, "uploadPart"), M = Math.ceil(t.size / b), w = new Array();
    for (let p = 0; p < M; ++p) w.push(t.slice(p * b, Math.min((p + 1) * b, t.size)));
    const y = w.slice().reverse(), F = new Array(), J = h(M, a?.onProgress, "uploadItem"), K = async () => {
      for (; y.length !== 0; ) {
    ...
    for (let p = 0; p < Fs && y.length !== 0; ++p) F.push(K());
    await Promise.all(F);
...
async function Is(t, { layer: e, ongoingUploads: s }, o) {
  ...
  if (Ns(t, e)) return o?.onProgress?.(1), t;
  const n = Es(t, e, o);
  s.set(t, n);
  try {
    await n;
...
async function Es(t, e, s) {
  const { metadata: o } = t, { displaySource: a } = o, n = q(a?.source, e), r = !!n, i = o.externalSources.length > 0, c = r ? Ss(n, e, s) : i ? ks(t, e, s) : xs(t, e, s), l = await c;
  return u(s), t.addExternalSources([l]), t;
}
async function Ss(t, e, s) {
  return { source: await C(t, e, s), original: !0 };
}
async function ks(t, e, s) {
  const o = z(e), { externalSources: a } = t.metadata, n = Us(a, e);
  if (!n) throw new ps();
  const r = h($.uploadConvertibleSource, s?.onProgress, "uploadConvertibleSource"), i = await C(n, e, { onProgress: r.makeOnProgress("uploadEditSource") });
...
async function xs(t, e, s) {
  const o = h($.uploadLocalMesh, s?.onProgress, "uploadLocalMesh"), a = Ds(t, e, { ...s, onProgress: o.makeOnProgress("meshToAssetBlob") });
...
async function R(t, e, s) {
  const o = h($.uploadAssetBlobs, s?.onProgress, "uploadAssetBlobs"), a = await Cs(t, e, { ...s, onProgress: o.makeOnProgress("prepareAssetItems") });
  u(s);
...
function Cs(t, e, s) {
  ...
  return Promise.all(t.map(async (a, n) => {
    const r = qs(await a, e, { ...s, onProgress: o.makeOnProgress(n) });
    return u(s), r;
  }));
}
...
//# sourceMappingURL=uploadAssets-B795ceBQ.js.map
