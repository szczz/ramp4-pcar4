import { a as I, f as V, l as X } from "./StyleDefinition-Dfu7Kx8O.js";
import { aM as b, mv as qt, mw as Kt, mx as Wt, my as me, mi as we } from "./main-lm2DHsZh.js";
import { L as W } from "./enums-qHpGJ28Q.js";
import { p as A, i as vt, b as Pt, I as Ee, N as re, P as Ie } from "./GeometryUtils-CFDcE7Ev.js";
import { C as g } from "./enums-DDkmfb-t.js";
import { t as v } from "./VertexElementDescriptor-BAy1DPb3.js";
import { R as be } from "./definitions-Doe0g1C2.js";
let Te = class {
  constructor(t) {
...
let j = class Dt {
  ...
  encodeAttributes(t, e, r, a) {
    ...
    for (const h of Object.keys(o)) {
      const p = o[h], { type: f, precisionFactor: d, isLayout: L } = i[h], $ = L ? r.getLayoutProperty(h) : r.getPaintProperty(h), z = $.interpolator?.getInterpolationRange(e);
      let M = 0;
      for (const T of p) {
  ...
  _buildAttributesInfo() {
    ...
    for (const p of o) {
      ...
      for (let z = 0; z < f; z++) {
        ...
        L.push({ dataIndex: R.dataIndex, offset: R.offset, bufferElementsToAdd: O }), R.offset += E, R.count += _;
      }
    }
...
`;
    ...
    for (const u of o) {
      l++;
      const { name: h, type: p, precisionFactor: f, isLayout: d } = n[u], L = f && f !== 1 ? " * " + 1 / f : "", { bytesPerElement: $, count: z } = Dt._encodingInfo[p], M = (T) => `a_data_${T.dataIndex}${ve(z, T.offset, $)}`;
...
`;
          ...
          a += `${this._getType(z)} ${h} = mix(${_}${L}, ${U}${L}, ${T});
`;
        }
      }
    }
...
const ne = { kind: "null" }, w = { kind: "number" }, S = { kind: "string" }, D = { kind: "boolean" }, Y = { kind: "color" }, bt = { kind: "object" }, C = { kind: "value" };
function ht(s, t) {
  return { kind: "array", itemType: s, n: t };
}
const De = [ne, w, S, D, Y, bt, ht(C)];
function rt(s) {
  if (s.kind === "array") {
...
function xt(s, t) {
  if (t.kind === "array") return s.kind === "array" && (s.n === 0 && s.itemType.kind === "value" || xt(s.itemType, t.itemType)) && (typeof t.n != "number" || t.n === s.n);
  if (t.kind === "value") {
    for (const e of De) if (xt(s, e)) return !0;
  }
...
class k {
  ...
  evaluate(t, e) {
    ...
    if (this._operator === "interpolate") {
      if (this.type.kind === "array" && Array.isArray(u) && Array.isArray(h)) return u.map((p, f) => A(p, h[f], l));
      if (this.type.kind === "color" && u instanceof b && h instanceof b) {
        const p = new b(u), f = new b(h);
        return new b([A(p.r, f.r, l), A(p.g, f.g, l), A(p.b, f.b, l), A(p.a, f.a, l)]);
      }
      if (this.type.kind === "number" && typeof u == "number" && typeof h == "number") return A(u, h, l);
      throw new Error(`"${this._operator}" cannot interpolate type ${rt(this.type)}`);
    }
    if (this._operator === "interpolate-hcl") {
      const p = qt(u), f = qt(h), d = f.h - p.h, L = Kt({ h: p.h + l * (d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d), c: A(p.c, f.c, l), l: A(p.l, f.l, l) });
      return new b({ a: A(u.a, h.a, l), ...L });
    }
    if (this._operator === "interpolate-lab") {
      const p = Wt(u), f = Wt(h), d = Kt({ l: A(p.l, f.l, l), a: A(p.a, f.a, l), b: A(p.b, f.b, l) });
      return new b({ a: A(u.a, h.a, l), ...d });
    }
...
class Me extends x {
  static parse(t, e) {
    switch (t.length) {
      case 2:
        return x.parse(t, e, (r) => -r[0]);
      case 3:
        return x.parse(t, e, (r) => r[0] - r[1]);
      default:
...
class Ye extends x {
  static parse(t, e) {
    if (t.length === 3) return x.parse(t, e, (r) => r[0] / r[1]);
...
class at {
  constructor(t, e) {
    this._arg = t, this._calculate = e, this.type = S;
  }
  static parse(t, e) {
...
class K {
  ...
  evaluate(t, e) {
    ...
    for (const a of this._args) {
      ...
      if (r = Tt(i), xt(r, this.type)) return i;
    }
...
class G {
  ...
  evaluate(t, e) {
    ...
    if (this.type === S) return Nt(this._args[0].evaluate(t, e));
    if (this.type === w) {
      for (const r of this._args) {
...
G.types = { "to-boolean": D, "to-color": Y, "to-number": w, "to-string": S };
class Ct {
  constructor(t) {
...
class It {
  constructor(t, e) {
    ...
    if (e == null && (e = t.default), Array.isArray(e) && e.length > 0 && _e[e[0]]) {
      const i = { number: w, color: Y, string: S, boolean: D, enum: S };
      try {
  ...
  _interpolate(t, e, r) {
    ...
    if (a && i) {
      ...
      if (Array.isArray(a[1])) {
        const h = a[1], p = i[1], f = [];
        for (let d = 0; d < h.length; d++) f.push(A(h[d], p[d], u));
        return f;
      }
      return A(a[1], i[1], u);
    }
...
//# sourceMappingURL=StyleRepository-BYhbefF3.js.map
