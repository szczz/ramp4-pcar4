...
import { bY as P, b_ as S, bZ as k } from "./main-lm2DHsZh.js";
class y extends P {
  get config() {
  ...
  async make(s) {
    ...
    if (p.isWebMercator())
      ...
      const o = new S("pole", { x: -96, y: 90 }), w = await this.$iApi.geo.proj.projectGeometry(p, o), a = this.$iApi.geo.map.mapPointToScreenPoint(w), c = {
        screenX: t.clientWidth / 2,
        screenY: t.clientHeight
      };
      r = Math.atan(
        (a.screenX - c.screenX) / (c.screenY - a.screenY)
      ) * 180 / Math.PI;
      const u = t.clientWidth / 2 + t.clientHeight * Math.tan(r * Math.PI / 180) - 50 / 2, x = Math.round(u / t.clientWidth * 100);
      h = Math.min(Math.max(x, 0), 100), l = (n) => {
        if (n < 0 || n > 100)
          throw new Error("Normalized position must be between 0 and 100");
        const b = t.clientWidth * n / 100;
        return Math.atan((a.screenX - b) / (50 / 2 - a.screenY)) * (180 / Math.PI);
      };
    }
    const g = {
      top: 0,
...
//# sourceMappingURL=index-NthhV6lY.js.map
