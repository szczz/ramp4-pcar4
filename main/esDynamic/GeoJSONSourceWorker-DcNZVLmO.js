import { de as _, df as R, s as I, dg as C, dh as Q, di as v, bg as Z, dj as A, M as G, D as P, K as M, dk as N, dl as z, $ as B, cV as $, dm as J, dn as L, dp as U } from "./main-lm2DHsZh.js";
import { m as V } from "./FeatureStore-kbC0mBT0.js";
import { x as E, j as b } from "./timeSupport-BMgFmqzA.js";
import { $ as H } from "./QueryEngine-cqgD_odw.js";
import { I as K, N as W, E as Y } from "./geojson-CvVDWXCl.js";
import { c as X, i as ee, o as te } from "./clientSideDefaults-BkFQkY5x.js";
import { j as se, p as T, d as w, f as F, y as k } from "./sourceUtils-VS0hDVPC.js";
...
class pe {
  ...
  async load(e, t = {}) {
    ...
    const i = [], [s] = await Promise.all([e.url ? this._fetch(t?.signal) : null, this._checkProjection(e.spatialReference)]), n = K(s, { geometryType: e.geometryType }), l = e.fields || n.fields || [], d = e.hasZ != null ? e.hasZ : n.hasZ, c = n.geometryType;
    ...
    for (const r of l) {
      ...
      if (!C.jsonValues.includes(r.type)) throw new I("geojson-layer:invalid-field-type", `invalid type for field "${r.name}"`, { field: r });
      if (r.name !== u.name) {
        const g = Q(r);
        g !== void 0 && (h[r.name] = g);
      }
      r.length == null && (r.length = v(r));
    }
    if (a) {
      if (a.startTimeField) {
    ...
    const p = c ? te(c) : void 0, j = o.dateFields.length ? { timeZoneIANA: Z } : null, f = { warnings: i, featureErrors: [], layerDefinition: { ...ie, drawingInfo: p ?? void 0, templates: X(h), extent: void 0, geometryType: c, objectIdField: y, fields: l, hasZ: !!d, timeInfo: a, dateFieldsTimeReference: j } };
    this._queryEngine = new H({ fieldsIndex: R.fromLayerJSON({ fields: l, timeInfo: a, dateFieldsTimeReference: j }), geometryType: c, hasM: !1, hasZ: d, objectIdField: y, spatialReference: m, timeInfo: a, featureStore: new V({ geometryType: c, hasM: !1, hasZ: d }), cacheSpatialQueries: !0 });
    ...
    const O = this._normalizeFeatures(x, f.featureErrors);
    this._queryEngine.featureStore.addMany(O);
    const { fullExtent: D, timeExtent: S } = await this._queryEngine.fetchRecomputedExtents();
    if (f.layerDefinition.extent = D, S) {
  ...
  async _createFeatures(e) {
    if (e == null) return [];
    const { geometryType: t, hasZ: i, objectIdField: s } = this._queryEngine, n = W(e, { geometryType: t, hasZ: i, objectIdField: s });
    ...
    for (const o of t) {
      ...
      if (h) i.push(h);
        ...
        if (o.geometry != null) {
          ...
          o.geometry = b(k(o.geometry, p), p, c);
        }
...
//# sourceMappingURL=GeoJSONSourceWorker-DcNZVLmO.js.map
