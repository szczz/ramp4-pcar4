import { s as f, D, aa as S } from "./main-lm2DHsZh.js";
...
const w = { sizeLo: 0, sizeHi: 4, minX: 8, minY: 16, minZ: 24, maxX: 32, maxY: 40, maxZ: 48, errorX: 56, errorY: 64, errorZ: 72, count: 80, reserved: 84, byteCount: 88 };
function k(t, e) {
  return { sizeLo: t.getUint32(e + w.sizeLo, d), sizeHi: t.getUint32(e + w.sizeHi, d), minX: t.getFloat64(e + w.minX, d), minY: t.getFloat64(e + w.minY, d), minZ: t.getFloat64(e + w.minZ, d), maxX: t.getFloat64(e + w.maxX, d), maxY: t.getFloat64(e + w.maxY, d), maxZ: t.getFloat64(e + w.maxZ, d), errorX: t.getFloat64(e + w.errorX, d), errorY: t.getFloat64(e + w.errorY, d), errorZ: t.getFloat64(e + w.errorZ, d), count: t.getUint32(e + w.count, d), reserved: t.getUint32(e + w.reserved, d) };
}
function q(t) {
  ...
  const n = k(e, r);
  if (r += w.byteCount, n.sizeHi * 2 ** 32 + n.sizeLo !== t.byteLength) throw new f("lepcc-decode-error", "Bad size");
  ...
  for (let b = 0; b < a.length; b++) {
    ...
    for (let C = 0; C < i[b]; C++) {
      v += u[g];
      const P = l[g];
      s[3 * g] = Math.min(n.maxX, n.minX + 2 * n.errorX * v), s[3 * g + 1] = Math.min(n.maxY, n.minY + 2 * n.errorY * y), s[3 * g + 2] = Math.min(n.maxZ, n.minZ + 2 * n.errorZ * P), g++;
    }
  }
...
function M(t, e, r) {
  ...
  const v = -1 >>> 32 - n;
  for (let C = 0; C < i; C++) {
    for (; y < n; ) g |= l[b] << y, y += 8, b += 1;
    r[C] = g & v, g >>>= n, y -= n, y + n > 32 && (g |= l[b - 1] >> 8 - y);
  }
  return e + b;
}
...
function H(t, e, r) {
  return T(t, e, r).map((c) => {
    ...
    return o == null || Number.isNaN(o) ? null : o;
  });
}
function T(t, e, r) {
  const c = [];
...
function J(t, e) {
  ...
  for (const u of e.ordering) {
    ...
    const l = { ...e.vertexAttributes[u], byteOffset: c, count: s }, g = R[u] || "_" + u;
    o.vertexAttributes[g] = l, c += A(l.valueType) * l.valuesPerElement * s;
  }
  const a = n.faceCount;
  if (e.faces && a) {
    o.faces = {};
...
//# sourceMappingURL=I3SBinaryReader-CSgjZ422.js.map
