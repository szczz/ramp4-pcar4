import { a9 as c, s as V, D as v } from "./main-lm2DHsZh.js";
import { L as o, k as $, q as k, E as M, y as U, z as L, B, C as F } from "./definitions-Doe0g1C2.js";
import { f as y } from "./UpdateTracking2D-BUCR6XDp.js";
import { d as g, c as R } from "./LabelMetric-pj4SxS9Q.js";
import { D as w, L as z } from "./enums-DDkmfb-t.js";
import { E as I } from "./Program-CfcTPgr7.js";
import { e as D, c as E } from "./Texture-DhDvGrgF.js";
import { i as O } from "./TileContainer-CEbFukYw.js";
const P = () => v.getLogger("esri.views.2d.engine.webgl.AttributeStoreView");
class m {
  constructor(t, e, i) {
  ...
  updateTexture(t, e) {
    try {
      ...
      const r = this.data.buffer, n = this.getTexture(t), a = 4, u = (i - i % this.size) / this.size, h = (s - s % this.size) / this.size, d = u, x = this.size, p = h, _ = u * this.size * a, f = (x + p * this.size) * a - _, b = g(this.pixelType), S = new b(r, _ * b.BYTES_PER_ELEMENT, f), A = this.size, T = p - d + 1;
      if (T > this.size) return void P().error(new V("mapview-webgl", "Out-of-bounds index when updating AttributeData"));
      n.updateData(0, 0, d, A, T, S);
    } catch {
    }
  }
  update(t) {
  ...
  _initTexture(t) {
    ...
    if (this._lastTexture != null && this._fbos[0]) {
      const i = this._lastTexture.descriptor.width, s = this._lastTexture.descriptor.height, r = this._lastTexture.descriptor.dataType, n = this._lastTexture.descriptor.pixelFormat, a = this.getFBO(t), u = R(r), h = new (g(r))(new ArrayBuffer(i * s * u * this.texelSize)), d = t.getBoundFramebufferObject(), { x, y: p, width: _, height: f } = t.getViewport();
      t.bindFramebuffer(a), a.readPixels(0, 0, i, s, n, r, h), e.updateData(0, 0, 0, 2 * i, s / 2, h), t.setViewport(x, p, _, f), t.bindFramebuffer(d);
    }
...
class q {
  ...
  _initialize(t) {
    ...
    if (this.size = t.blockSize, c("esri-2d-update-debug") && console.debug("AttributeStoreView.initialize", { message: t }), this._data == null) this._data = e.map((i, s) => i != null ? new m(i, this.size, s) : null);
    else for (let i = 0; i < this._data.length; i++) {
      const s = this._data[i], r = e[i];
      r != null && (s == null ? this._data[i] = new m(r, this.size, i) : s.resize(r, this.size));
    }
  }
  destroy() {
  ...
  getVVSize(t) {
    return this.getData(t, o.VV, 0, 0);
  }
  getData(t, e, i, s) {
  ...
  requestUpdate(t) {
    this._version = t.version, this._pendingAttributeUpdates.push(t), c("esri-2d-update-debug") && console.debug(`Version[${this._version}] AttributeStoreView.requestUpdate`, { message: t });
  }
  get currentEpoch() {
    return this._epoch;
  }
  update() {
    ...
    for (const e of t) {
      ...
      c("esri-2d-update-debug") && console.debug(`Version[${this._version}] Epoch[${r}] AttributeStoreView.applyUpdate`), this._version = n, this._epoch = r, s != null && this._initialize(s);
      const a = this._data;
      for (let u = 0; u < i.length; u++) {
        const h = i[u], d = a[u];
        d != null && h != null && (c("esri-2d-update-debug") && console.debug(`Version[${this._version}] CpuBlock[${u}] AttributeStoreView.update`, { block: h }), d.update(h));
      }
    }
  }
  getUniforms(t) {
    return { filterFlags: { texture: this._getTexture(t, o.FilterFlags), unit: $ }, animation: { texture: this._getTexture(t, o.Animation), unit: k }, gpgpu: { texture: this._getTexture(t, o.GPGPU), unit: M }, visualVariableData: { texture: this._getTexture(t, o.VV), unit: U }, dataDriven0: { texture: this._getTexture(t, o.DD0), unit: L }, dataDriven1: { texture: this._getTexture(t, o.DD1), unit: B }, dataDriven2: { texture: this._getTexture(t, o.DD2), unit: F }, size: this.size };
  }
  _getTexture(t, e) {
  ...
  _getDefaultTexture(t) {
    if (this._defaultTexture == null) {
      const e = new D();
      e.wrapMode = w.CLAMP_TO_EDGE, e.samplingMode = z.NEAREST, e.width = 1, e.height = 1, this._defaultTexture = new E(t, e, new Uint8Array(4));
    }
...
class X {
  ...
  createMeshInfo(t) {
    return { id: this._instanceId, techniqueType: this.techniqueRef.type, inputParams: t, optionalAttributes: this._input.optionalAttributes };
  }
  getInput() {
    return this._input;
  }
...
//# sourceMappingURL=TechniqueInstance-DfC9gsuC.js.map
