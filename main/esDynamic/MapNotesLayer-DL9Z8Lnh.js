import { dS as T, O as o, P as a, Q as C, eh as G, ee as B, eg as L, eI as F, e3 as k, e7 as z, e5 as K, e6 as W, e8 as A, bx as b, V as O, eJ as H, aa as w, dd as I, eK as N, eL as h, eM as M, eN as $, aj as Q, s as V, eO as q, K as U, br as Z, bn as X, bo as Y, bp as ee, ai as te, dL as v, e9 as c, eH as re, b8 as oe, eP as le, eQ as R, ei as ie } from "./main-lm2DHsZh.js";
...
const x = new T({ name: "OBJECTID", alias: "OBJECTID", type: "oid", nullable: !1, editable: !1 }), ne = new T({ name: "title", alias: "Title", type: "string", nullable: !0, editable: !0, length: 255 });
let u = class extends le {
  ...
  get fullExtent() {
    ...
    return t ? e == null ? h(E(), t).geometry : $(e, t) : null;
  }
  get fullBounds() {
...
let i = class extends k(z(K(W(A(ie))))) {
  constructor(t) {
  ...
  readSublayers(t, e, r) {
    ...
    for (const n of e.layers) {
      const { layerDefinition: y, featureSet: p } = n, m = y.id ?? s++, d = D(n);
      if (d != null) {
        const f = new u({ id: d.id, title: y.name, layerId: m, layer: this, graphics: p.features.map(({ geometry: S, symbol: j, attributes: P, popupInfo: _ }) => Q.fromJSON({ attributes: P, geometry: S, symbol: j, popupTemplate: _ })) });
        l.push(f);
      }
    }
    return new O(l);
  ...
  writeSublayers(t, e, r, l) {
    ...
    if (!this.capabilities.operations.supportsMapNotesEditing) return void (y && l?.messages?.push(new V("map-notes-layer:editing-not-supported", "New map notes cannot be added to this layer")));
    const p = [];
  ...
  async beforeSave() {
    ...
    for (const l of this.sublayers) for (const s of l.graphics) if (s.geometry != null) {
      const n = s.geometry;
      t ? U(n.spatialReference, t) || (Z(n.spatialReference, t) || X() || await Y(), s.geometry = ee(n, t)) : t = n.spatialReference, e.push(s);
    }
  ...
  _writeMapNoteSublayer(t, e, r, l, s, n, y) {
    ...
    if (e != null) {
      ...
      this._normalizeObjectIds(p, x), t.push({ layerDefinition: { name: e.title, drawingInfo: { renderer: { type: "simple", symbol: w(r.identifyingSymbol) } }, id: e.layerId, geometryType: r.geometryTypeJSON, minScale: l, maxScale: s, objectIdField: "OBJECTID", fields: [x.toJSON(), ne.toJSON()], spatialReference: n }, featureSet: { features: p, geometryType: r.geometryTypeJSON } });
    }
  }
  _writeMapNote(t, e, r, l) {
    ...
    if (s == null) return;
    if (s.type !== r) return void l?.messages?.push(new v("map-notes-layer:invalid-geometry-type", `Geometry "${s.type}" cannot be saved in "${r}" layer`, { graphic: e }));
    if (n == null) return void l?.messages?.push(new v("map-notes-layer:no-symbol", "Skipping map notes with no symbol", { graphic: e }));
...
//# sourceMappingURL=MapNotesLayer-DL9Z8Lnh.js.map
