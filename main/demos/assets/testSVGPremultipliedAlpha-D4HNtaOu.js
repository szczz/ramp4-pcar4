import { m } from './WGLContainer-BXJm6cE9.js';
import { E as E$1, c as c$1 } from './Program-Dw204J9G.js';
import { D, L, F, _ as _$1, E as E$2, G, U } from './enums-CwcDCDam.js';
import { e as e$1, c } from './Texture-BcGpRBCj.js';
import { o } from './ProgramTemplate-BUEXYvVH.js';
import { fI as r$1 } from './main-ZcwzHdcj.js';
...
See https://js.arcgis.com/4.30/esri/copyright.txt for details.
*/
class f extends t{constructor(e){super(),this._rctx=e;const r="\n    precision highp float;\n\n    attribute vec2 a_pos;\n    varying vec2 v_uv;\n\n    void main() {\n      v_uv = a_pos;\n      gl_Position = vec4(a_pos * 2.0 - 1.0, 0.0, 1.0);\n    }\n    ",t="\n    precision highp float;\n\n    varying vec2 v_uv;\n\n    uniform sampler2D u_texture;\n\n    void main() {\n      gl_FragColor = texture2D(u_texture, v_uv);\n    }\n    ";this._program=e.programCache.acquire(r,t,new Map([["a_pos",0]]));}dispose(){super.dispose();}_test(g){const f=this._rctx;if(!f.gl)return g.dispose(),!0;const l=new e$1(1);l.wrapMode=D.CLAMP_TO_EDGE,l.samplingMode=L.NEAREST;const w=new E$1(f,l),v=c$1.createVertex(f,F.STATIC_DRAW,new Uint16Array([0,0,1,0,0,1,1,1])),x=new o(f,new Map([["a_pos",0]]),m,{geometry:v}),h=new e$1;h.samplingMode=L.LINEAR,h.wrapMode=D.CLAMP_TO_EDGE;const b=new c(f,h,_);f.useProgram(g),f.bindTexture(b,0),g.setUniform1i("u_texture",0);const A=f.getBoundFramebufferObject(),{x:E,y:T,width:j,height:C}=f.getViewport();f.bindFramebuffer(w),f.setViewport(0,0,1,1),f.setClearColor(0,0,0,0),f.setBlendingEnabled(!1),f.clear(_$1.COLOR_BUFFER_BIT),f.bindVAO(x),f.drawArrays(E$2.TRIANGLE_STRIP,0,4);const y=new Uint8Array(4);return w.readPixels(0,0,1,1,G.RGBA,U.UNSIGNED_BYTE,y),x.dispose(),w.dispose(),b.dispose(),f.setViewport(E,T,j,C),f.bindFramebuffer(A),255!==y[0]}}const _=new Image;_.src="data:image/svg+xml,%3C%3Fxml version='1.0' encoding='UTF-8'%3F%3E%3Csvg width='5' height='5' version='1.1' viewBox='0 0 5 5' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='5' height='5' fill='%23f00' fill-opacity='.5'/%3E%3C/svg%3E%0A",_.width=5,_.height=5,_.decode();
