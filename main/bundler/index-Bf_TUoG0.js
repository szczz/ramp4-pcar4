import { defineComponent as pe, inject as de, useTemplateRef as ue, reactive as me, watch as te, onMounted as ye, onUnmounted as we, openBlock as ge, createElementBlock as he, markRaw as fe } from "vue";
import { u as Y } from "./draw-store-BEUmwV44.js";
import "tiny-emitter";
import { G as re, L as ve, F as be } from "./main-CKWgYI6e.js";
...
const Ae = (I, r, A) => {
  ...
  return i ? typeof i == "function" ? i() : Promise.resolve(i) : new Promise((t, n) => {
    (typeof queueMicrotask == "function" ? queueMicrotask : setTimeout)(
      n.bind(
        null,
        new Error(
          "Unknown variable dynamic import: " + r + (r.split("/").length !== A ? ". Note that variables only represent file names one level deep." : "")
        )
      )
    );
  });
  ...
  setup(I) {
    ...
    const ne = async () => {
      ...
      _.x += b, _.y += w;
      const $ = T.toMap(_), z = $.x - l.x, U = $.y - l.y;
      let g;
      if (s.type === "point")
        !p && !m ? g = new W({
          x: s.x + z,
        ...
        if (!p && !m) {
          const h = s.paths.map((R) => R.map(([D, k]) => [D + z, k + U]));
          g = new M({
            paths: h,
            spatialReference: s.spatialReference
          });
        ...
        if (!p && !m) {
          const h = s.rings.map((R) => R.map(([D, k]) => [D + z, k + U]));
          g = new E({
            rings: h,
            spatialReference: s.spatialReference
          });
...
class ut extends xe {
  async init() {
    ...
    this.$vApp.$el.getElementsByClassName("inner-shell")[0].appendChild(r.childNodes[0]), Object.entries(Pe).forEach((t) => this.$iApi.$i18n.mergeLocaleMessage(...t)), this._parseConfig(this.config), Y(this.$vApp.$pinia).supportedTypes.forEach((t) => {
      const n = `${t.type}-icon`;
      this.$iApi.component(n, fe(Ae(/* @__PURE__ */ Object.assign({ "./icons/circle-icon.vue": () => import("./circle-icon-DmAXJJ65.js"), "./icons/edit-icon.vue": () => import("./edit-icon-BiZww--3.js"), "./icons/point-icon.vue": () => import("./point-icon-BqJVtDz2.js"), "./icons/polygon-icon.vue": () => import("./polygon-icon-BqHy8sNn.js"), "./icons/polyline-icon.vue": () => import("./polyline-icon-CJTWWQE2.js"), "./icons/rectangle-icon.vue": () => import("./rectangle-icon-C0kIr6mp.js") }), `./icons/${t.type}-icon.vue`, 3)));
    });
  }
  async added() {
