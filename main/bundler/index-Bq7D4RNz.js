import "tiny-emitter";
import { F as C, s as y, l as G, G as T, P as W, t as z, L as B, v as H, w as R } from "./main-CKWgYI6e.js";
...
import { defineComponent as q, inject as j, ref as r, computed as E, reactive as O, onMounted as X, onBeforeUnmount as F, openBlock as V, createElementBlock as D, normalizeStyle as K, createElementVNode as U } from "vue";
import "pinia";
import "vue-i18n";
import "screenfull";
...
const Q = "path", Z = 12, ee = "M14.4 6L14 4H5v17h2v-7h5.6l.4 2h7V6z", te = "#ff0000ff", oe = 5, re = 6, ie = {
  ...
  setup(S) {
    ...
    X(() => {
      ...
      $.value = i.tileSchemas, n?.value && (P.value = `<img width='25' src='${n.value}'>`), e.geo.map.esriView?.ready && I(e.geo.map.getExtent()), A.push(e.event.on(T.MAP_EXTENTCHANGE, Y(300, I)));
    }), F(() => {
    ...
    const I = async (i) => {
      ...
      const l = e.$vApp.$el.querySelector(".inner-shell"), d = f.value.querySelector(".northarrow").getBoundingClientRect().width, u = e.$vApp.$el.querySelector(".appbar")?.clientWidth || 0, x = i.sr;
      if (h || typeof h > "u" && !x.isWebMercator()) {
        const m = new W("pole", { x: -96, y: 90 }), L = await e.geo.proj.projectGeometry(x, m), v = e.geo.map.mapPointToScreenPoint(L);
        if (v.screenY < 0) {
          c.value = !0;
          const a = {
