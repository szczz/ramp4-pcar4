import { Q as e, s as t } from "./main-CPEpcI7m.js";
import { merge as n } from "es-toolkit";
import { fabric as r } from "fabric";
//#region src/fixtures/export-northarrow/index.ts
var i = class extends t {
	get config() {
		return this.$iApi.fixture.get("export").config?.mapElements;
	}
	async make(t) {
		let i = () => 0, a = 0, o = this.$vApp.$el.querySelector(".inner-shell"), s = this.$iApi.geo.map.getSR(), c = 0;
		if (s.isWebMercator()) c = 0;
		else {
			let t = new e("pole", {
				x: -96,
				y: 90
			}), n = await this.$iApi.geo.proj.projectGeometry(s, t), r = this.$iApi.geo.map.mapPointToScreenPoint(n), l = {
				screenX: o.clientWidth / 2,
				screenY: o.clientHeight
			};
			c = Math.atan((r.screenX - l.screenX) / (l.screenY - r.screenY)) * 180 / Math.PI;
			let u = o.clientWidth / 2 + o.clientHeight * Math.tan(c * Math.PI / 180) - 50 / 2, d = Math.round(u / o.clientWidth * 100);
			a = Math.min(Math.max(d, 0), 100), i = (e) => {
				if (e < 0 || e > 100) throw Error("Normalized position must be between 0 and 100");
				let t = o.clientWidth * e / 100;
				return Math.atan((r.screenX - t) / (50 / 2 - r.screenY)) * (180 / Math.PI);
			};
		}
		let l = {
			top: 0,
