import { defineComponent as ie, ref as b, watch as ye, toRef as je, openBlock as v, createElementBlock as g, createElementVNode as n, toDisplayString as y, unref as o, inject as ge, useTemplateRef as ee, reactive as re, onMounted as he, onBeforeUnmount as ce, resolveDirective as Oe, createCommentVNode as L, createVNode as $, withCtx as C, withDirectives as be, normalizeClass as ne, createTextVNode as we, Fragment as Ye, renderList as Ke, vShow as Ae, Transition as Je, renderSlot as Te, computed as H, provide as Ze, useId as Qe, onErrorCaptured as Xe, resolveComponent as et, createBlock as q, withModifiers as $e, nextTick as tt } from "vue";
import { ColorPicker as lt } from "vue-accessible-color-picker";
import { _ as ve, I as at, L as S, a2 as K, G as Ie, a0 as pe } from "./main-CKWgYI6e.js";
...
const st = { class: "flex justify-end mb-20" }, nt = ["disabled", "animation"], rt = { class: "button-text" }, it = /* @__PURE__ */ ie({
  ...
  setup(s) {
    ...
    return (h, f) => (v(), g("div", st, [
      n("button", {
        ...
        type: "button",
        onClick: f[0] || (f[0] = (I) => h.$emit("cancel"))
  ...
  key: 1,
  class: "text-red-900 text-xs"
}, At = { key: 3 }, $t = ["aria-label"], It = { class: "text-base font-bold" }, Ot = { key: 4 }, Tt = { class: "text-base font-bold" }, Ct = { class: "relative mb-0.5" }, Mt = ["value", "aria-label"], Rt = {
  key: 0,
  class: "text-red-900 text-xs"
}, Nt = /* @__PURE__ */ ie({
  __name: "form-input",
  ...
  setup(s, { expose: c, emit: l }) {
    const w = ge("iApi"), { t: E } = Se(), h = l, f = s, I = b(), V = ee("textInput"), N = ee("selectInput"), _ = ee("urlInput");
    ...
    const a = () => {
      ...
      I.value.style.height = `${i + p + 30}px`;
    };
    G.push(
      ye(T, (u) => {
        u && de();
  ...
  setup(s, { emit: c }) {
    ...
    const I = () => l.activeIndex > h.value, V = () => l.activeIndex === h.value;
    return (N, _) => {
      const z = Oe("truncate");
      return v(), g("div", Ut, [
        n("div", _t, [
          I() ? (v(), g("div", Bt, _[0] || (_[0] = [
            n("svg", {
              xmlns: "http://www.w3.org/2000/svg",
              height: "100%",
              width: "100%",
  ...
  setup(s) {
    const c = at(), { t: l } = Se(), w = ge("iApi"), E = b(), h = Qe(), f = b([]), I = H(() => c.layerSource), V = H(() => c.currStep), N = b(), _ = b(0), z = b(!1), D = b(), W = ee("stepOneStart"), te = ee("stepTwoStart"), B = ee("stepThreeStart"), P = b(!1), J = b(!1), M = b(!1), T = b(!1), G = b(!1), Z = b(!1), le = b(!0), ue = b(""), Q = b(""), x = b([]), ae = re([
      {
        value: S.FEATURE,
    ...
    const p = async (e) => {
      ...
      e?.preventDefault(), k.value = I.value.guessFormatFromURL(A.value), c.goToStep(K.FORMAT);
    }, xe = async (e) => {
      ...
      try {
        a.value = Y() ? await I.value.fetchFileInfo(A.value, k.value, j.value) : await I.value.fetchServiceInfo(
          A.value,
          k.value,
          c.nested
      ...
      }), d.userAdded = !0, w.event.emit(Ie.USER_LAYER_ADDED, d), M.value = !1, c.goToStep(K.UPLOAD);
      ...
      if (c.nested = e, x.value = [], _.value += 1, k.value === S.MAPIMAGE) {
        a.value.layers = I.value.createLayerHierarchy(
          a.value.layersRaw,
          c.nested
        );
        const t = new Set(
        ...
        a.value.layers = I.value.mapWmsLayerList(
          a.value.layersRaw,
          c.nested
        );
      ...
      for (const r of e.value.layersRaw)
        if (r.parentLayerId !== -1) {
          const O = d.get(r.parentLayerId) || [];
          O.push(r.id), d.set(r.parentLayerId, O);
        }
      const m = (r) => {
        const O = d.get(r);
        return O ? O.every((X) => d.has(X) ? m(X) : t.has(X)) : !1;
      }, F = (r) => {
        if (m(r))
          x.value.push(r);
        else {
      ...
      const d = (r) => !r.layers || r.layers.length === 0 ? t.has(r.name) : r.layers.every((O) => d(O)), m = (r) => {
      ...
      for (const F of t) {
        const r = m.layers.find((O) => O.name === F);
