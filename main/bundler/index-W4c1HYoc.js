import { defineComponent as _, inject as E, ref as u, reactive as d, onMounted as w, onBeforeUnmount as x, openBlock as y, createElementBlock as A, createElementVNode as M, toDisplayString as T, unref as I } from "vue";
import { G as s, _ as P, F as R } from "./main-CKWgYI6e.js";
import "pinia";
      ...
      t.geo.map.viewPromise.then(() => {
        r.push(
          ...
          t.geo.map.esriView.on("pointer-move", (o) => {
            const { pointerId: h, pointerType: f, x: v, y: g } = o, c = e.get(h);
            if (!c || f !== "touch" || e.size !== 1) {
              i.value.classList.remove("pg-active");
              return;
            }
            Math.sqrt(Math.pow(v - c.x, 2) + Math.pow(g - c.y, 2)) < 20 || (i.value.classList.add("pg-active"), n.value !== -1 && clearTimeout(n.value), n.value = window.setTimeout(() => {
              i.value.classList.remove("pg-active");
            }, 2e3));
          })
        );
...
}), $ = /* @__PURE__ */ P(V, [["__scopeId", "data-v-22ab9ef7"]]);
class _e extends R {
  added() {
