import { bB as FixtureInstance, i6 as useLegendStore, iD as SectionItem, iE as LayerItem, iF as LegendItem, iG as LayerType, iH as LayerInstance, bG as defineComponent, bH as useI18n, bI as inject, bN as resolveComponent, bP as openBlock, bQ as createBlock, bR as withCtx, bU as unref, fH as createBaseVNode, he as markRaw, hf as useAppbarStore, hg as useMapnavStore } from './main-75ed2508.js';
import { _ as _sfc_main$1 } from './screen.vue_vue_type_script_setup_true_lang-cbc19196.js';
...
class LegendAPI extends FixtureInstance {
  ...
  async addLayerItem(layer, parent) {
    const item = new LayerItem(
      ...
      {
        layerId: layer.id,
        sublayerIndex: layer.layerIdx !== -1 ? layer.layerIdx : void 0,
        name: layer.name
      },
      parent
    );
  ...
  getLayerItem(layer) {
    ...
    legend.some((item) => {
      result = this._searchTree(
        item,
        (item2) => item2 instanceof LayerItem && item2.layerId === id
      );
      return result !== void 0;
    });
    if (result === void 0) {
      ...
      legend.some((item) => {
        result = this._searchTree(
          item,
          (item2) => item2 instanceof LayerItem && item2.layerUid === id
        );
        return result !== void 0;
      });
    }
  ...
  getAllExpanded(expanded) {
    ...
    legend.forEach((item) => {
      items.push(
        ...this._searchTreeAll(item, (item2) => {
          return item2.children.length > 0 && item2.expanded === check;
        })
      );
    });
    return items;
   ...
   */
  updateLegend(layer) {
    const updateLayerItem = (layer2, error) => {
      const layerItem = this.getLayerItem(layer2);
      if (error) {
        if (layerItem && layer2 instanceof LayerInstance) {
          layerItem.layer = layer2;
        }
        layerItem?.error();
      } else {
        layerItem?.load(
          layer2 instanceof LayerInstance ? layer2 : void 0
        );
      }
    };
        ...
        const treeParser = (node) => {
          if (node.isLayerRoot && !node.isLogicalLayer) {
            ...
            if (layerItem && !layerItem.treeGrown) {
              node.children.map(
                (childNode) => this._treeWalker(layer, childNode)
              ).map(
                (childConf) => this.addItem(
                  childConf,
                  layerItem
                )
              );
            ...
            node.children.forEach(
              (childNode) => treeParser(childNode)
            );
   ...
   * @memberof LegendAPI
   */
  reloadLayerItem(layerId) {
    const item = this.getLayerItem(layerId);
    if (!item) {
      return false;
    }
  ...
  _deleteItem(item) {
    ...
    const removeItemAndDescendants = (itemToRemove) => {
      ...
      if (itemToRemove instanceof LayerItem) {
        itemToRemove.handlers.forEach(
          (handler) => this.$iApi.event.off(handler)
        );
      }
  ...
  _treeWalker(layer, node, extraConfig) {
    ...
    if (node.isLayerRoot && !node.isLogicalLayer) {
      ...
      currItem.children = node.children.map(
        (childNode) => this._treeWalker(layer, childNode, extraConfig)
      );
    } else if (!node.isLayerRoot && !node.isLogicalLayer) {
      ...
      currItem.sublayerIndex = layer.layerIdx === -1 ? void 0 : layer.layerIdx;
    }
...
class LegendFixture extends LegendAPI {
  added() {
    ...
    this._parseConfig(
      this.config !== void 0 ? JSON.parse(JSON.stringify(this.config)) : void 0
    );
    const unwatch = this.$vApp.$watch(
      () => this.config,
      (value) => this._parseConfig(
        value !== void 0 ? JSON.parse(JSON.stringify(value)) : void 0
      )
    );
    this.removed = () => {
      unwatch();
